...
//		MeccreateRing(pBlock, pRingForm->inner_diam);
//		pRingForm->close();
//		pBlock->close();
//	});
//
...
void FingerProsthetic_Designer(){
	...
	QObject::connect(pMainForm->pPushBtnCreate, &QPushButton::clicked, [&]() {
		...
		if (pMainForm->pCGBoxIndex->pCheckBoxFinger->isChecked()) {
			...
			mech.setPosRing(mech, 0, fingerJointCenter.x-0.5);

			mech.pRing->close();

		}

		if (pMainForm->pCGBoxMiddle->pCheckBoxFinger->isChecked()) {
		...
		if (pMainForm->pCGBoxRing->pCheckBoxFinger->isChecked()) {
		...
		if (pMainForm->pCGBoxPinky->pCheckBoxFinger->isChecked()) {
